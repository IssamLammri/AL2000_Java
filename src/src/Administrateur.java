...
//		Catalogue C = new Catalogue(1, 1);
		Film F = new Film();
		DemandesFilm = F.GetAllDemandesFilms();
		boolean Test;
		...
		for (Film e : DemandesFilmTree) {
			System.out.println(
					"    " + a + "    |    " + NombreOccurance.get(f) + "    |  " + e.getTitre_Film() + "        ");
			System.out.println("---------|---------|------------------------|");
			f++;
			a++;
		}
...
//		Catalogue C = new Catalogue(1, 1);
		...
		Film F = new Film();
		FilmEx = F.GetAllDemandesFilms();
		 
		System.out.println();
		System.out.println("Si Vous Voulez supprimer un Film Taper '1' et Si cous Voulez vider la table Taper '0' ? ");
		System.out.println();
	...
	public ArrayList<Location> RécupérerDVDLouerActuelement() {
		...
		Location L = new Location();
		Loca = L.GetAllLocations();
		for (Iterator iterator = Loca.iterator(); iterator.hasNext();) {

			Location location = (Location) iterator.next();
			if (location.getDate_Rendu() != null) {
				Loca.remove(iterator);
			}
		}
		...
		for (Location e : Loca) {
			System.out.println(
					"     " + e.getNumero_Location() + "       |    " + shortDateFormat.format(e.getDate_Location())
							+ "    |  " + e.getClient().getNom() + "      |   " + e.getDvd().getFilm().getTitre_Film());
			System.out.println(
					"--------------------------------------------------------------------------------------------");
			a++;
		}
	...
	public void ConsulterFilmLouéParMois(Date date) {
		Calendar cal1 = Calendar.getInstance();
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		ArrayList<Location> Loca = new ArrayList();
