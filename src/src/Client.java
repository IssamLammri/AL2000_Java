...
public class Client implements Serializable {
			...
			if (function_NBLocationParClient() < 3) {
				if (Function_ExistFILMINAL20000(dvd)) {
					if (dvd.getQuantite() > 0) {
						...
						Location Loc = new Location(date, null, true, 4, this, dvd);
	...
	public void Sabonner() {
		...
		ArrayList<Object> abns = abT.GetAllClients();
		if (abns != null) {
			boolean etat1 = false;
			while (!etat1) {
				boolean etat2 = true;
		...
		Abonne ab = new Abonne(this.Nom, this.Prenom, this.Adresse, this.Date_N, this.CB, username, password, date,
				Carte_Abon);
		Clients.remove(this);
		SerializableClients();
	}
	...
	public void Rendre_DVD(int NumeroLoca) {
		...
		Boolean etat = false;
		for (Location location : Locations) {
			if (location.getNumero_Location() == NumeroLoca && location.getDate_Rendu() == null) {
				Date date = new Date();
				...
				for (Object client : Clients) {
					if (((Client) client).getCB().getTitulaire_Carte()
							.equals(location.getClient().getCB().getTitulaire_Carte())
							&& ((Client) client).getNom().equals(location.getClient().getNom())) {
						Double MontantCar = ((Client) client).getCB().getMontant() - prix;
						((Client) client).getCB().setMontant(MontantCar);
					}
				}

				for (DVD dvd2 : dvds) {
					if (location.getDvd().getFilm().getTitre_Film().equals(dvd2.getFilm().getTitre_Film())) {
						dvd2.setQuantite(dvd2.getQuantite() + 1);
					}
				}
				dvd.setDVD(dvds);
		...
		if (!etat)
			System.out.println("Location introuvable ou  film deja rendu !!! ");
	...
	public String toString() {
		return "Client [ getNom()=" + getNom() + ", getPrenom()=" + getPrenom() + ", getAdresse()=" + getAdresse()
				+ ", getDate_N()=" + getDate_N() + "]";
	}
